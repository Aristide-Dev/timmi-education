...
export default function AvailabilityEdit({ teacher, availability }: Props) {
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Disponibilités', href: teachersRoutes.availability.edit.url(teacher.id) },
  ];

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={`Disponibilités - ${teacher.name}`} />
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Définissez les jours et horaires où {teacher.name} est disponible pour les cours
          </p>
        </div>
        <Button
          variant="outline"
          onClick={() => router.visit(teachersRoutes.show.url(teacher.id))}
      ...
      <form
        onSubmit={(e) => {
          e.preventDefault();
          form.put(teachersRoutes.availability.update.url(teacher.id), {
            preserveScroll: true,
          });
        }}
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Ajoutez un ou plusieurs créneaux pour chaque jour. Laissez vide si le professeur n'est pas disponible ce jour-là.
            </CardDescription>
          </CardHeader>
